...
// ================================================================
// PHASE 1: Priority Countries (Test First)
// ================================================================

const COUNTRIES = [
  'russia', 'south-africa', 'france', 'greece', 'spain',
  'uk', 'usa', 'canada', 'australia', 'uae',
  'thailand', 'singapore'
];
...
function getAI(): GoogleGenAI {
  if (!aiInstance) {
    if (!process.env.GEMINI_API_KEY) {
      throw new Error('GEMINI_API_KEY environment variable is not set. Please set it before running heal.');
    }
...
// ================================================================
// 3. COUNTRY-SPECIFIC RULES (CORRECTED)
// ================================================================
...
function generateBatchReport(results: any[]) {
...
<body>
  ...
  <div class="card">
    <h2>📝 Summary</h2>
    <ul>
      <li>✅ Healed: ${healed.length}</li>
      <li>⏭️ Skipped: ${skipped.length}</li>
...
async function main() {
  switch (command) {
    ...
    default:
      console.log(`
📋 Visa Data Batch Healing System (Production-Ready)

Commands:
  npm run visa:batch:duplicate    # Create test copies (Phase 1: ${COUNTRIES.length} countries)
...
✅ CORRECTED STATUTORY FEES:
  France: 90 EUR (was 80 EUR)
  Greece: 90 EUR (was 80 EUR)
  UK: £127 (was £115)
  Australia: 195 AUD (was 190 AUD)
  Greece: GVCW (was VFS Global)

✅ SAFEGUARDS:
  - Rate Limit: 2-second delay between API calls
  - Progress Bar: Clean display, no console clash
  - File Parsing: Preserves TypeScript format
  - Prompt: Universal + Country-specific rules

Phase 1 Countries: ${COUNTRIES.length} countries
      `);
  }
}
